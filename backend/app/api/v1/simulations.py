...
from app.schemas.runway_conversion_target import RunwayConversionTargetOut
...
from app.schemas.runway_price_target import RunwayPriceTargetOut
...
from app.simulation.runway_conversion_target import build_runway_conversion_target
...
from app.simulation.runway_price_target import build_runway_price_target
...
@router.get(
    "/{simulation_id}/runway-price-target",
    response_model=RunwayPriceTargetOut,
    summary=(
        "Find the minimum average order value that preserves cash until break-even"
    ),
    responses=_JSON_200,
)
def get_runway_price_target(
    simulation_id: int,
    starting_cash: float = Query(
        CASH_RUNWAY_DEFAULT_STARTING_CASH,
        ge=0.0,
    ...
    planned_average_order_value: float = Query(
        BREAK_EVEN_DEFAULT_AVERAGE_ORDER_VALUE,
        ge=0.0,
        le=BREAK_EVEN_MAX_AVERAGE_ORDER_VALUE,
        description="Planned average revenue per purchase",
    ),
    gross_margin: float = Query(
        BREAK_EVEN_DEFAULT_GROSS_MARGIN,
        ge=0.0,
    ...
    """Turn a completed simulation into a cash-safe average order value target.

    The solver searches average order value in exact cent increments and
    reports the smallest price that reaches operating break-even within the
    chosen horizon without any month ending cash-negative. It compares that
    target with the founder's planned price and quantifies either price
    headroom or the required increase. Pure post-hoc analytics: no Celery,
    LLM, or database writes.
    """
    sim = _get_owned_simulation(simulation_id, current_user.id, db)
    ...
    if sim.status == "FAILED":
        raise HTTPException(
            status_code=422,
            detail="Simulation failed; runway price analysis is unavailable.",
        )
    if sim.status != "COMPLETED":
        raise HTTPException(
            status_code=409,
            detail=(
                f"Simulation is {sim.status} — runway price analysis "
                "requires completed results."
            ),
        )
    if not sim.results_json:
    ...
    return build_runway_price_target(
        sim.results_json,
        simulation_id=sim.id,
        project_id=sim.project_id,
        status=sim.status,
...
@router.get(
    "/{simulation_id}/runway-conversion-target",
    response_model=RunwayConversionTargetOut,
    summary=(
        "Find the minimum conversion rate that preserves cash until break-even"
    ),
    responses=_JSON_200,
)
def get_runway_conversion_target(
    simulation_id: int,
    starting_cash: float = Query(
        CASH_RUNWAY_DEFAULT_STARTING_CASH,
        ge=0.0,
    ...
    """Turn a completed simulation into a cash-safe conversion target.

    The solver searches visitor-to-purchase conversion in exact 0.00000001
    increments and reports the smallest rate that reaches operating break-even
    within the chosen horizon without any month ending cash-negative. It
    compares that target with the simulation prediction and quantifies both
    the percentage-point gap and relative lift required. Pure post-hoc
    analytics: no Celery, LLM, or database writes.
    """
    sim = _get_owned_simulation(simulation_id, current_user.id, db)
    ...
    if sim.status != "COMPLETED":
        raise HTTPException(
            status_code=409,
            detail=(
                f"Simulation is {sim.status} — runway conversion analysis "
                "requires completed results."
            ),
        )
    if not sim.results_json:
    ...
    return build_runway_conversion_target(
        sim.results_json,
        simulation_id=sim.id,
        project_id=sim.project_id,
        status=sim.status,
...
@router.get(
    "/{simulation_id}/runway-funding-target",
    response_model=RunwayFundingTargetOut,
    summary="Find the minimum opening cash needed to preserve runway to break-even",
    responses=_JSON_200,
)
def get_runway_funding_target(
    simulation_id: int,
    planned_starting_cash: float = Query(
        CASH_RUNWAY_DEFAULT_STARTING_CASH,
        ge=0.0,
        le=CASH_RUNWAY_MAX_STARTING_CASH,
        description="Planned cash available at the start of month 1",
    ),
    horizon_months: int = Query(
        CASH_RUNWAY_DEFAULT_HORIZON_MONTHS,
        ge=CASH_RUNWAY_MIN_HORIZON_MONTHS,
    ...
    """Turn a completed simulation into an opening-cash funding target.

    The calculation finds the cash trough of the planned operating ledger and
    reports the exact opening balance needed to reach monthly break-even inside
    the chosen horizon without ending any month cash-negative. It compares the
    target with the founder's planned cash and quantifies the funding gap or
    surplus. Pure post-hoc analytics: no Celery, LLM, or database writes.
    """
    sim = _get_owned_simulation(simulation_id, current_user.id, db)
    ...
    if sim.status == "FAILED":
        raise HTTPException(
            status_code=422,
            detail="Simulation failed; runway funding analysis is unavailable.",
        )
    if sim.status != "COMPLETED":
        raise HTTPException(
            status_code=409,
            detail=(
                f"Simulation is {sim.status} — runway funding analysis "
                "requires completed results."
            ),
        )
    if not sim.results_json:
    ...
    return build_runway_funding_target(
        sim.results_json,
        simulation_id=sim.id,
        project_id=sim.project_id,
        status=sim.status,
        planned_starting_cash=planned_starting_cash,
        horizon_months=horizon_months,
        initial_monthly_visitors=initial_monthly_visitors,
        monthly_visitor_growth_rate=monthly_visitor_growth_rate,
        monthly_fixed_costs=monthly_fixed_costs,
...
@router.get(
    "/{simulation_id}/runway-growth-target",
    response_model=RunwayGrowthTargetOut,
    summary=(
        "Find the minimum monthly visitor growth that reaches break-even before cash runs out"
    ),
    responses=_JSON_200,
)
def get_runway_growth_target(
    simulation_id: int,
    starting_cash: float = Query(
        CASH_RUNWAY_DEFAULT_STARTING_CASH,
        ge=0.0,
    ...
    planned_monthly_visitor_growth_rate: float = Query(
        CASH_RUNWAY_DEFAULT_VISITOR_GROWTH,
        ge=0.0,
        le=CASH_RUNWAY_MAX_VISITOR_GROWTH,
        description="Founder's planned compounded monthly visitor growth",
    ),
    monthly_fixed_costs: float = Query(
        BREAK_EVEN_DEFAULT_MONTHLY_FIXED_COSTS,
        ge=0.0,
    ...
    """Turn a completed simulation into a cash-safe visitor-growth target.

    The solver searches from zero to 100% compounded monthly visitor growth
    and reports the smallest rate that reaches operating break-even within the
    chosen horizon without any month ending cash-negative. It compares that
    target with the founder's plan and explains when growth alone cannot close
    the runway gap. Pure post-hoc analytics: no Celery, LLM, or database writes.
    """
    sim = _get_owned_simulation(simulation_id, current_user.id, db)
    ...
    if sim.status != "COMPLETED":
        raise HTTPException(
            status_code=409,
            detail=(
                f"Simulation is {sim.status} — runway growth targeting requires completed results."
            ),
        )
    if not sim.results_json:
        raise HTTPException(
    ...
    return build_runway_growth_target(
        sim.results_json,
        simulation_id=sim.id,
        project_id=sim.project_id,
        status=sim.status,
        ...
        horizon_months=horizon_months,
        initial_monthly_visitors=initial_monthly_visitors,
        planned_monthly_visitor_growth_rate=(planned_monthly_visitor_growth_rate),
        monthly_fixed_costs=monthly_fixed_costs,
        average_order_value=average_order_value,
        gross_margin=gross_margin,
        purchases_per_customer_per_month=purchases_per_customer_per_month,
...
@router.get(
    "/{simulation_id}/runway-spend-ceiling",
    response_model=RunwaySpendCeilingOut,
    summary=(
        "Find the maximum recurring fixed costs that preserve cash until break-even"
    ),
    responses=_JSON_200,
)
def get_runway_spend_ceiling(
    simulation_id: int,
    starting_cash: float = Query(
        CASH_RUNWAY_DEFAULT_STARTING_CASH,
        ge=0.0,
    ...
    planned_monthly_fixed_costs: float = Query(
        BREAK_EVEN_DEFAULT_MONTHLY_FIXED_COSTS,
        ge=0.0,
        le=BREAK_EVEN_MAX_MONTHLY_FIXED_COSTS,
        description="Planned recurring monthly costs before acquisition spend",
    ),
    average_order_value: float = Query(
        BREAK_EVEN_DEFAULT_AVERAGE_ORDER_VALUE,
        ge=0.0,
    ...
    """Turn a completed simulation into a cash-safe recurring spend limit.

    The solver searches monthly fixed costs in exact cent increments and
    reports the largest budget that reaches operating break-even within the
    chosen horizon without any month ending cash-negative. It compares that
    ceiling with the founder's plan and quantifies either monthly headroom or
    the required cost reduction. Pure post-hoc analytics: no Celery, LLM, or
    database writes.
    """
    sim = _get_owned_simulation(simulation_id, current_user.id, db)
    ...
    if sim.status != "COMPLETED":
        raise HTTPException(
            status_code=409,
            detail=(
                f"Simulation is {sim.status} — runway spending analysis "
                "requires completed results."
            ),
        )
    if not sim.results_json:
    ...
    return build_runway_spend_ceiling(
        sim.results_json,
        simulation_id=sim.id,
        project_id=sim.project_id,
        status=sim.status,
        ...
        initial_monthly_visitors=initial_monthly_visitors,
        monthly_visitor_growth_rate=monthly_visitor_growth_rate,
        planned_monthly_fixed_costs=planned_monthly_fixed_costs,
        average_order_value=average_order_value,
        gross_margin=gross_margin,
        purchases_per_customer_per_month=purchases_per_customer_per_month,
        cost_per_visitor=cost_per_visitor,
