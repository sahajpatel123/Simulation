...
from app.simulation.description_export import description_count_to_csv, description_to_csv
...
def export_brief_positioning(
    ...
    if fmt == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="brief-positioning-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
@router.get(
    "/{project_id}/description-count/export",
    summary="Export a project's description count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_description_count(
    project_id: int,
    ...
    """Export a project's description presence as CSV (default) or JSON."""
    project = get_owned_project(db, current_user.id, project_id)
    description = getattr(project, "description", None)
    count = 1 if isinstance(description, str) and description.strip() else 0
    row = {"project_id": project_id, "description_count": count}
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="description-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )

    csv_text = description_count_to_csv(
        row,
        metadata={
            "generated_at": datetime.now(UTC).isoformat(),
            "user_id": current_user.id,
...
@router.get(
    "/{project_id}/title/export",
    summary="Export a project's title as CSV",
    response_class=StreamingResponse,
)
def export_title(
    project_id: int,
    ...
    """Spreadsheet export of a project's title field."""
    project = get_owned_project(db, current_user.id, project_id)
...
@router.get(
    "/{project_id}/is-archived/export",
    summary="Export a project's is_archived flag as CSV",
    response_class=StreamingResponse,
)
def export_is_archived(
    ...
    """Spreadsheet export of a project's is_archived flag."""
    project = get_owned_project(db, current_user.id, project_id)
...
@router.get(
    "/{project_id}/created-at/export",
    summary="Export a project's created_at as CSV",
    response_class=StreamingResponse,
)
def export_created_at(
    ...
    """Spreadsheet export of a project's created_at field."""
    project = get_owned_project(db, current_user.id, project_id)
...
@router.get(
    "/{project_id}/status/export",
    summary="Export a project's status as CSV",
    response_class=StreamingResponse,
)
def export_status(
    project_id: int,
    ...
    """Spreadsheet export of a project's status field."""
    project = get_owned_project(db, current_user.id, project_id)
...
def export_landing_url(
    ...
    if fmt == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="landing-page-url-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
def export_existing_product(
    ...
    if fmt == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="existing-product-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
def export_precis_fingerprint(
    ...
    if fmt == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="precis-fingerprint-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
@router.get(
    "/{project_id}/assumption-count/export",
    summary="Export a project's assumption count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_assumption_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
            "Output format. ``csv`` (default) returns the "
            "spreadsheet-friendly table; ``json`` returns the raw "
            "assumption-count row."
        ),
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> StreamingResponse:
    """Export a project's assumption count as CSV (default) or JSON."""
    get_owned_project(db, current_user.id, project_id)
    count = db.query(Assumption).filter(Assumption.project_id == project_id).count()
    row = {"project_id": project_id, "assumption_count": count}
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="assumption-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
@router.get(
    "/{project_id}/evidence-count/export",
    summary="Export a project's evidence count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_evidence_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
...
@router.get(
    "/{project_id}/outcome-tracker-count/export",
    summary="Export a project's outcome tracker count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_outcome_tracker_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="outcome-tracker-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="outcome-tracker-count-{project_id}.csv"'
            ),
            "Content-Length": str(len(body)),
        },
    )
...
@router.get(
    "/{project_id}/premortem-count/export",
    summary="Export a project's premortem count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_premortem_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
    ...
    """Export a project's premortem count as CSV (default) or JSON."""
    project = get_owned_project(db, current_user.id, project_id)
    count = 1 if getattr(project, "premortem_json", None) else 0
    row = {"project_id": project_id, "premortem_count": count}
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="premortem-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
@router.get(
    "/{project_id}/competitive-count/export",
    summary="Export a project's competitive count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_competitive_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
            "Output format. ``csv`` (default) returns the "
            "spreadsheet-friendly table; ``json`` returns the raw "
            "competitive-count row."
        ),
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> StreamingResponse:
    """Export a project's competitive count as CSV (default) or JSON."""
    project = get_owned_project(db, current_user.id, project_id)
    data = getattr(project, "competitive_json", None) or {}
    count = len(data.get("competitors", []) or [])
    row = {"project_id": project_id, "competitive_count": count}
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="competitive-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
@router.get(
    "/{project_id}/mvp-feature-count/export",
    summary="Export a project's MVP feature count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_mvp_feature_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
            "Output format. ``csv`` (default) returns the "
            "spreadsheet-friendly table; ``json`` returns the raw "
            "mvp-feature-count row."
        ),
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> StreamingResponse:
    """Export a project's MVP feature count as CSV (default) or JSON."""
    project = get_owned_project(db, current_user.id, project_id)
    count = len(project.mvp_feature_list or [])
    row = {"project_id": project_id, "mvp_feature_count": count}
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="mvp-feature-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
...
@router.get(
    "/{project_id}/brief-completed-count/export",
    summary="Export a project's brief-completed count as CSV or JSON",
    response_class=StreamingResponse,
)
def export_brief_completed_count(
    project_id: int,
    format: Literal["csv", "json"] = Query(
        default="csv",
        description=(
            "Output format. ``csv`` (default) returns the "
            "spreadsheet-friendly table; ``json`` returns the raw "
            "brief-completed-count row."
        ),
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> StreamingResponse:
    """Export a project's brief-completed count as CSV (default) or JSON."""
    project = get_owned_project(db, current_user.id, project_id)
    count = 1 if getattr(project, "brief_completed_at", None) else 0
    row = {"project_id": project_id, "brief_completed_count": count}
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": (
                    f'attachment; filename="brief-completed-count-{project_id}.json"'
                ),
                "Content-Length": str(len(body)),
            },
        )
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="brief-completed-count-{project_id}.csv"'
            ),
            "Content-Length": str(len(body)),
        },
    )
