...
def scenarios_with_negative_pct(scenarios: list[WhatIfOut]) -> list[WhatIfOut]:
    """Return scenarios whose ``conversion_delta_pct`` is strictly negative."""
    return [scenario for scenario in scenarios if scenario.conversion_delta_pct < 0.0]
...
def scenarios_with_any_category_count(
    scenarios: list[WhatIfOut],
    categories: list[str],
) -> int:
    """Return the number of scenarios whose meta matches **any** of the categories.

    Equivalent to ``len(scenarios_with_any_category(scenarios, categories))``
    without the intermediate list allocation.
    """
    return sum(1 for scenario in scenarios if scenario.has_category_any(categories))


def count_with_matched_categories(
    scenarios: list[WhatIfOut],
    categories: list[str],
) -> int:
    """Return the number of scenarios whose meta matches **all** of the categories.

    Equivalent to ``len(scenarios_with_matched_category_count(scenarios, categories))``
    without the intermediate list allocation.
    """
    return sum(1 for scenario in scenarios if scenario.has_all_categories(categories))
