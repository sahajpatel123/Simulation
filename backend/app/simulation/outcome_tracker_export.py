...
def outcome_tracker_count_to_csv(
    row: dict[str, Any],
    metadata: dict[str, Any] | None = None,
) -> str:
    """Render an outcome-tracker-count row as a single-row CSV table."""
    buffer = io.StringIO()
    writer = csv.writer(buffer, lineterminator="\n")
...
__all__ = ["outcome_tracker_count_to_csv", "outcome_tracker_to_csv"]
