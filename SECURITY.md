...
```bash
# resolve + emit hashes: gen_dependency_lock.py <pip-report.json> <out.lock> <title>
pip install --dry-run --only-binary :all: --report r.json \
    $(grep -v '^#' requirements.txt | tr '\n' ' ')
python tools/gen_dependency_lock.py r.json requirements-lock.txt runtime
pip install --dry-run --require-hashes -r requirements-lock.txt
```
