"""Route-level tests for the /me/portfolio-health-snapshot cache.
...
modules (users.py clear-archive, simulations.py,
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_portfolio_health_snapshot(
        db=_FakeSession(),
        current_user=u,
    )


def test_portfolio_health_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_portfolio_health_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_portfolio_health_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_portfolio_health_namespace_constant_export():
...
def test_portfolio_health_namespace_consistency_across_modules():
    ...
    namespace = users_mod._USER_PORTFOLIO_HEALTH_SNAPSHOT_CACHE_NAMESPACE
    assert namespace == "user-portfolio-health-snapshot"
    ...
    for src, label in (
        ...
        assert (
            "_USER_PORTFOLIO_HEALTH_SNAPSHOT_CACHE_NAMESPACE" in s
        ), (
            f"_USER_PORTFOLIO_HEALTH_SNAPSHOT_CACHE_NAMESPACE "
