"""Route-level tests for the /projects/{id}/intake-mode/export endpoint."""
...
def _call_route(
    *,
    ...
    return proj_mod.export_intake_mode(
...
def test_export_intake_mode_returns_csv() -> None:
    resp = _call_route()

    assert resp.media_type == "text/csv; charset=utf-8"
    assert 'filename="intake-mode-10.csv"' in resp.headers["Content-Disposition"]
    body = _body(resp).decode("utf-8")
    assert "project_id,intake_mode" in body
    assert "10,IDEA" in body
    assert "user_id,42" in body


def test_export_intake_mode_format_json_returns_payload() -> None:
    ...
    assert '"intake_mode": "IDEA"' in body


def test_export_intake_mode_missing_project_raises_404() -> None:
