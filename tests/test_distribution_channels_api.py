"""Route-level tests for the /simulations/{id}/distribution-channels endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_distribution_channels(
...
def test_completed_hardware_simulation_returns_distribution_payload(
    ...
    assert 0.0 <= out.weighted_accessibility <= 1.0
...
def test_unsupported_product_type_returns_insufficient_data(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_health_hardware_is_supported(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _FakeSession(
        _FakeSimulation(
            results={
                "population_weighted_conversion": 0.04,
                "product_type_detected": "health_hardware",
            }
        )
    )
    out = _call_route(session=session, monkeypatch=monkeypatch)
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("Sold through offline retail and tier-2 stores"),
        ],
    )
    ...
    assert any("offline retail" in a["text"] for a in run["assumptions"])
    assert run["product_type"].value == "consumer_hardware"


def test_unknown_product_type_falls_back_to_saas_insufficient_read(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _FakeSession(
        _FakeSimulation(
