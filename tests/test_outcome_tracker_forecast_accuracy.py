"""Tests for historical trajectory-forecast accuracy verification."""
...
from app.schemas.outcome_tracker import OutcomeTrackerForecastAccuracyOut
...
def _linear_rows(
    ...
    return [
        _row(i + 1, day=day, rate=start + step * day)
...
def _call(
    ...
    *,
    ...
    return build_outcome_tracker_forecast_accuracy(
        rows,
...
def test_short_span_cannot_verify_any_horizon() -> None:
    ...
    rows = [
        ...
        _row(2, day=1.0, rate=0.06),
        _row(3, day=2.0, rate=0.07),
    ]
...
def test_perfect_linear_series_scores_accurate() -> None:
    ...
    """
    out = _call(_linear_rows(), predicted=0.05)
    ...
    for horizon in by_horizon.values():
        assert horizon["within_2pp_rate"] == 1.0
...
def test_decelerating_series_is_over_predicted() -> None:
    """Actuals that fall after a strong early trend must flag over-prediction."""
    rows = [
        _row(1, day=0.0, rate=0.05),
        _row(2, day=30.0, rate=0.11),
        _row(3, day=60.0, rate=0.11),
        _row(4, day=90.0, rate=0.09),
        _row(5, day=120.0, rate=0.08),
        _row(6, day=150.0, rate=0.07),
    ]
    out = _call(rows, predicted=0.05)
    assert out["total_verifications"] >= 4
    assert out["overall_bias_direction"] == BIAS_OVER_PREDICTS
    assert out["overall_bias"] is not None and out["overall_bias"] > 0.02
...
def test_malformed_rows_are_dropped() -> None:
    rows = [
        ...
        _row(7, day=30.0, rate=0.08),
        _row(8, day=60.0, rate=0.11),
        _row(9, day=90.0, rate=0.14),
        _row(10, day=120.0, rate=0.17),
        _row(11, day=150.0, rate=0.20),
        _row(12, day=180.0, rate=0.23),
    ]
    clean = _call(
        [_row(6, day=0.0, rate=0.05), _row(7, day=30.0, rate=0.08),
         _row(8, day=60.0, rate=0.11), _row(9, day=90.0, rate=0.14),
         _row(10, day=120.0, rate=0.17), _row(11, day=150.0, rate=0.20),
         _row(12, day=180.0, rate=0.23)],
        predicted=0.05,
    )
    out = _call(rows, predicted=0.05)
...
def test_duplicate_timestamps_keep_last_row() -> None:
    rows = [
        ...
        _row(2, day=30.0, rate=0.17),
        _row(3, day=60.0, rate=0.14),
        _row(4, day=90.0, rate=0.11),
        _row(5, day=120.0, rate=0.08),
        _row(6, day=150.0, rate=0.05),
        _row(7, day=180.0, rate=0.02),
        _row(8, day=180.0, rate=0.05),  # same timestamp, later rate wins
    ]
    expected = [
        _row(1, day=0.0, rate=0.20),
        _row(2, day=30.0, rate=0.17),
...
def test_extreme_values_stay_finite_and_bounded() -> None:
    """Zero actuals and near-ceiling rates must not produce NaN scores."""
    rows = [
        _row(1, day=0.0, rate=0.05),
...
def _call_route(
    *,
    ...
    return mod.get_outcome_tracker_forecast_accuracy(
        project_id=7,
...
def test_route_falls_back_to_row_prediction_when_no_simulation() -> None:
    ...
    for row in rows:
        row.predicted_conversion_rate = 0.05
