"""Route-level tests for the /me/last-touched-project cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_last_touched_project(
        db=_FakeSession(),
        current_user=u,
    )


def test_last_touched_caches_payload(monkeypatch):
    pytest.importorskip(
    ...
    assert out1.has_activity is False
...
def test_last_touched_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_last_touched_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_last_touched_namespace_constant_export():
...
def test_last_touched_namespace_consistency_across_modules():
    ...
    namespace = users_mod._USER_LAST_TOUCHED_PROJECT_CACHE_NAMESPACE
    assert namespace == "user-last-touched-project"
    ...
    for src, label in (
        ...
        assert (
            "_USER_LAST_TOUCHED_PROJECT_CACHE_NAMESPACE" in s
        ), (
            f"_USER_LAST_TOUCHED_PROJECT_CACHE_NAMESPACE "
