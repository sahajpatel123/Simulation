"""Route-level tests for the /simulations/{id}/cultural-fit endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_cultural_fit(
...
def test_completed_simulation_returns_cultural_fit_payload(
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("The app ships in Hindi and regional languages"),
        ],
    )
    ...
    assert any("Hindi" in a["text"] for a in run["assumptions"])
