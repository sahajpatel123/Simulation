"""Route-level tests for the /simulations/{id}/ecosystem-compatibility
endpoint.
"""
...
def _real_hardware_conductor_result() -> object:
    """One real deterministic consumer_hardware run, shared by all route
    tests."""
...
def _real_saas_conductor_result() -> object:
    """Real saas run — EcosystemCompatibilityArchitect does not activate
    there, so it produces an INSUFFICIENT_DATA read."""
    conductor = _RealConductor()
    return conductor.run(
        agents=[],
        env_params={
...
class _StubConductor:
    ...
    def run(self, agents, env_params, assumptions, product_type=None, **kwargs):
        ...
        if product_type is not None and product_type.value == "saas":
            return _real_saas_conductor_result()
        return _real_hardware_conductor_result()
...
def _call_route(
    *,
    ...
    return sim_mod.get_ecosystem_compatibility(
...
def test_completed_hardware_simulation_returns_payload(
...
def test_smart_home_is_supported_end_to_end() -> None:
    """smart_home is a newer product type; run the real conductor (no
    stub) so activation is actually exercised."""
...
def test_saas_product_type_returns_insufficient_data(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _FakeSession(
        _FakeSimulation(
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("The device ships with Matter and an open API"),
        ],
    )
    ...
    assert any("Matter" in a["text"] for a in run["assumptions"])
