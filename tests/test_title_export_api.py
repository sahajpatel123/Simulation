"""Route-level tests for the /projects/{id}/title/export endpoint."""
...
def test_export_title_returns_csv() -> None:
    resp = _call_route()

    assert resp.media_type == "text/csv; charset=utf-8"
    assert 'filename="title-10.csv"' in resp.headers["Content-Disposition"]
    body = _body(resp).decode("utf-8")
    assert "project_id,title" in body
    assert "10,TheCee" in body
    assert "user_id,42" in body


def test_export_title_format_json_returns_payload() -> None:
...
def test_export_title_missing_project_raises_404() -> None:
