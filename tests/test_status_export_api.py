"""Route-level tests for the /projects/{id}/status/export endpoint."""
...
def test_export_status_returns_csv() -> None:
    resp = _call_route()

    assert resp.media_type == "text/csv; charset=utf-8"
    assert 'filename="status-10.csv"' in resp.headers["Content-Disposition"]
    body = _body(resp).decode("utf-8")
    assert "project_id,status" in body
    assert "10,DRAFT" in body
    assert "user_id,42" in body


def test_export_status_format_json_returns_payload() -> None:
...
def test_export_status_missing_project_raises_404() -> None:
