"""Route-level tests for the /me/runs-per-week cache.
...
sites (simulations.py, users.py clear-archive).
"""
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_runs_per_week(
        db=_FakeSession(),
        current_user=u,
    )


def test_runs_per_week_caches_payload(monkeypatch):
    pytest.importorskip(
    ...
    out1 = _call_route()
    assert out1.total_simulations == 0
...
def test_runs_per_week_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_runs_per_week_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_runs_per_week_namespace_constant_export():
...
def test_runs_per_week_namespace_consistency_across_modules():
    """Pin that the 2 invalidation sites (simulations.py,
    users.py clear-archive) import and use the constant
    ...
    namespace = users_mod._USER_RUNS_PER_WEEK_CACHE_NAMESPACE
    assert namespace == "user-runs-per-week"
    ...
    for src, label in (
        ...
        assert (
            "_USER_RUNS_PER_WEEK_CACHE_NAMESPACE" in s
        ), (
            f"_USER_RUNS_PER_WEEK_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
