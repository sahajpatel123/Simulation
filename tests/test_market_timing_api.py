"""Route-level tests for the /simulations/{id}/market-timing endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_market_timing(
...
def test_completed_simulation_returns_market_timing_payload(
    ...
    assert 0.0 <= out.timing_index <= 1.0
    ...
    assert sum(out.gate_distribution.values()) > 0.99
    assert out.primary_gate in {
        "regulatory",
        "category_awareness",
        "problem_urgency",
        "category_education_cost",
        "switching_cost",
        "technology_adoption",
        "budget_cycle",
    }
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("Customers urgently need this today"),
        ],
    )
    ...
    assert any("urgently need" in a["text"] for a in run["assumptions"])
