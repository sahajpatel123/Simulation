"""Route-level tests for the /me/decision-rate cache.
...
sites (simulations.py, decisions.py, users.py clear-archive).
"""
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_decision_rate(
        db=_FakeSession(),
        current_user=u,
    )


def test_decision_rate_caches_payload(monkeypatch):
    pytest.importorskip(
    ...
    out1 = _call_route()
    # Empty list under the fake query mocks.
...
def test_decision_rate_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_decision_rate_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_decision_rate_namespace_constant_export():
...
def test_decision_rate_namespace_consistency_across_modules():
    """Pin that the 3 invalidation sites (simulations.py,
    decisions.py, users.py clear-archive) import and use
    ...
    namespace = users_mod._USER_DECISION_RATE_CACHE_NAMESPACE
    assert namespace == "user-decision-rate"
    ...
    for src, label in (
        ...
        assert (
            "_USER_DECISION_RATE_CACHE_NAMESPACE" in s
        ), (
            f"_USER_DECISION_RATE_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
