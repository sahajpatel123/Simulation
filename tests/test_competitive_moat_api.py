"""Route-level tests for the /simulations/{id}/competitive-moat endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_competitive_moat(
...
def test_completed_simulation_returns_competitive_moat_payload(
    ...
    assert 0.0 <= out.moat_index <= 1.0
    assert len(out.cluster_profiles) >= 50
    assert len(out.top_protected) <= 5
    assert len(out.top_vulnerable) <= 5
    ...
    assert out.primary_weakest_lever in {
        "feature_parity",
        "brand_trust",
        "pricing_power",
        "distribution_reach",
    ...
    assert sum(out.lever_distribution.values()) > 0.99
...
def test_unknown_product_type_falls_back_to_saas(
    ...
    session = _FakeSession(
        _FakeSimulation(
            results={
                "population_weighted_conversion": 0.04,
                "product_type_detected": "not_a_real_type",
            }
        )
    )
    out = _call_route(session=session, monkeypatch=monkeypatch)
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("Our product is 10x better than the incumbent"),
        ],
    )
    ...
    assert any("10x better" in a["text"] for a in run["assumptions"])
