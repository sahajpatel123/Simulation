"""Route-level tests for the /me/decision-velocity cache.
...
modules (simulations.py, decisions.py).
"""
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_decision_velocity(
        db=_FakeSession(),
        current_user=u,
    )


def test_decision_velocity_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_decision_velocity_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_decision_velocity_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_decision_velocity_namespace_constant_export():
...
def test_decision_velocity_namespace_consistency_across_modules():
    """Pin that both invalidation sites (simulations.py
    POST /simulations, decisions.py POST /decisions) import
    ...
    namespace = users_mod._USER_DECISION_VELOCITY_CACHE_NAMESPACE
    assert namespace == "user-decision-velocity"
    ...
    for src, label in (
        ...
        assert (
            "_USER_DECISION_VELOCITY_CACHE_NAMESPACE" in s
        ), (
            f"_USER_DECISION_VELOCITY_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
