"""Route-level tests for the /projects/{id}/competitive-analysis/export endpoint."""
...
def _call_route(
    *,
    ...
    return proj_mod.export_competitive_analysis(
...
def test_export_competitive_analysis_returns_csv() -> None:
    resp = _call_route()

    assert resp.media_type == "text/csv; charset=utf-8"
    assert 'filename="competitive-10.csv"' in resp.headers["Content-Disposition"]
    body = _body(resp).decode("utf-8")
    assert "name,category,pricing,positioning" in body
    assert "Acme,DIRECT,$10/mo,premium" in body
    assert "user_id,42" in body


def test_export_competitive_analysis_format_json_returns_payload() -> None:
...
def test_export_competitive_analysis_empty_returns_header_only() -> None:
    session = _FakeSession(_EmptyProject())
    resp = _call_route(session=session)

    body = _body(resp).decode("utf-8")
    assert "name,category,pricing,positioning" in body
    assert "Acme,DIRECT" not in body


def test_export_competitive_analysis_missing_project_raises_404() -> None:
