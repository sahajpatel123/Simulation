"""Route-level tests for the /me/outcome-rate cache.
...
sites (simulations.py, outcomes.py x3, users.py
clear-archive).
"""
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_outcome_rate(
        db=_FakeSession(),
        current_user=u,
    )


def test_outcome_rate_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_outcome_rate_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_outcome_rate_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_outcome_rate_namespace_constant_export():
...
def test_outcome_rate_namespace_consistency_across_modules():
    """Pin that the 4 invalidation sites (simulations.py,
    outcomes.py x3 via replace_all, users.py clear-archive)
    ...
    namespace = users_mod._USER_OUTCOME_RATE_CACHE_NAMESPACE
    assert namespace == "user-outcome-rate"
    ...
    for src, label in (
        ...
        assert (
            "_USER_OUTCOME_RATE_CACHE_NAMESPACE" in s
        ), (
            f"_USER_OUTCOME_RATE_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
