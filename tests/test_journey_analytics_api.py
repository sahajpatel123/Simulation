"""
Route-level tests for ``GET /api/v1/simulations/{id}/journey``.
...
from app.schemas.journey_analytics import (  # noqa: E402
    JourneyAnalyticsOut,
)
...
def _call_route(
    *,
    ...
    return sim_mod.get_simulation_journey_analytics(
...
def test_completed_simulation_returns_journey_analytics() -> None:
    out = _call_route()
    ...
    assert out.abandon_probability == pytest.approx(1.0 - 0.060933, abs=1e-5)
    assert out.top_paths
    assert out.leverage_rankings
    assert out.per_cluster
    assert out.key_insights
    assert out.meta["matrix_count"] == 2
...
def test_missing_journey_data_raises_404() -> None:
    session = _FakeSession(
        _FakeSimulation(
            results={
