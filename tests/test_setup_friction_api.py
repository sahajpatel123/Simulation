"""Route-level tests for the /simulations/{id}/setup-friction endpoint.
"""
...
def _real_conductor_result() -> object:
    ...
    by all route tests that exercise the setup stack."""
...
def _real_saas_conductor_result() -> object:
    """One real deterministic saas conductor run (no SetupFirstUseArchitect
    metrics), used for unknown / non-hardware product-type fallback."""
    conductor = _RealConductor()
    return conductor.run(
        agents=[],
        env_params={
...
def _call_route(
    *,
    ...
    return sim_mod.get_setup_friction(
...
def test_completed_simulation_returns_setup_friction_payload(
    ...
    assert out.meta["requires_companion_app"] is False
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("companion app required for first-time setup"),
        ],
    )
    ...
    assert any("companion app" in a["text"] for a in run["assumptions"])
