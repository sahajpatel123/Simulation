"""Route-level tests for the /projects/{id}/health cache.
...
def _call_route(
    current_user_id: int = 42, project_id: int = 1,
):
    from app.api.v1 import projects as proj_mod

    return proj_mod.get_project_health(
...
def test_project_health_caches_payload(monkeypatch) -> None:
    pytest.importorskip(
...
def test_project_health_cache_isolated_per_user(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_project_health_cache_isolated_per_project(
    monkeypatch,
) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
...
def test_project_health_noop_when_redis_down(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_project_health_namespace_constant_export() -> None:
    """Pin that the constant is importable from projects.py
    so the invalidator modules can use it without
    touching internal attribute access."""
    from app.api.v1.projects import (
        _PROJECT_HEALTH_CACHE_NAMESPACE,
    )

    assert (
        _PROJECT_HEALTH_CACHE_NAMESPACE == "project-health"
    )


def test_project_health_namespace_consistency_across_modules() -> None:
    ...
    for src, label in (
        ...
        assert (
            "_PROJECT_HEALTH_CACHE_NAMESPACE" in s
        ), (
            f"_PROJECT_HEALTH_CACHE_NAMESPACE not imported "
            f"in {label}"
        )
        assert (
