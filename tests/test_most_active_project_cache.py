"""Route-level tests for the /me/most-active-project cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_most_active_project(
        db=_FakeSession(),
        current_user=u,
    )


def test_most_active_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_most_active_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_most_active_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_most_active_namespace_constant_export():
...
def test_most_active_namespace_consistency_across_modules():
    ...
    namespace = users_mod._USER_MOST_ACTIVE_PROJECT_CACHE_NAMESPACE
    assert namespace == "user-most-active-project"
    ...
    for src, label in (
        ...
        assert (
            "_USER_MOST_ACTIVE_PROJECT_CACHE_NAMESPACE" in s
        ), (
            f"_USER_MOST_ACTIVE_PROJECT_CACHE_NAMESPACE "
