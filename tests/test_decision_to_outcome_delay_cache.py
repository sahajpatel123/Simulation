"""Route-level tests for the /me/decision-to-outcome-delay cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_decision_to_outcome_delay(
        db=_FakeSession(),
        current_user=u,
    )


def test_decision_to_outcome_delay_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_decision_to_outcome_delay_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_decision_to_outcome_delay_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_decision_to_outcome_delay_namespace_constant_export():
...
def test_decision_to_outcome_delay_namespace_consistency_across_modules():
    """Pin that the 3 invalidator modules (decisions.py,
    outcomes.py, users.py clear-archive) import and use
    ...
    for src, label in (
        ...
        assert (
            "_USER_DECISION_TO_OUTCOME_DELAY_CACHE_NAMESPACE" in s
        ), (
            f"_USER_DECISION_TO_OUTCOME_DELAY_CACHE_NAMESPACE "
