"""Route-level tests for the /projects/{id}/premortem-digest cache.

Pins: cache hit short-circuit, per-user + per-project key
isolation, Redis-down no-op, and namespace-string
consistency between the read path and the
run_premortem invalidation site.
"""
...
class _FakeProject:
    """Minimal stand-in that supplies ``premortem_json``."""
...
def _call_route(current_user_id: int = 42, project_id: int = 1):
    ...
    return proj_mod.get_premortem_digest(
...
def test_premortem_digest_caches_payload(monkeypatch) -> None:
    pytest.importorskip(
...
def test_premortem_digest_cache_isolated_per_project(
    monkeypatch,
) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
...
def test_premortem_digest_noop_when_redis_down(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_premortem_digest_namespace_consistency() -> None:
    """The namespace string in projects.py is the single
    source of truth — both the read path and the
    invalidation site must reference it.
    """
    from app.api.v1 import projects as proj_mod

    namespace = proj_mod._PREMORTEM_DIGEST_CACHE_NAMESPACE
    assert namespace == "project-premortem-digest"
