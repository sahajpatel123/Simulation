"""Route-level tests for the /me/projects-needing-attention cache.
...
modules (simulations.py, decisions.py, outcomes.py x3,
users.py clear-archive, projects.py extract-assumptions).
"""
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_projects_needing_attention(
        db=_FakeSession(),
        current_user=u,
    )


def test_projects_needing_attention_caches_payload(monkeypatch):
    pytest.importorskip(
    ...
    out1 = _call_route()
    assert out1.needing_attention_count == 0
...
def test_projects_needing_attention_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_projects_needing_attention_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_projects_needing_attention_namespace_constant_export():
...
def test_projects_needing_attention_namespace_consistency_across_modules():
    """Pin that the 4 invalidation modules import and use
    ...
    namespace = users_mod._USER_PROJECTS_NEEDING_ATTENTION_CACHE_NAMESPACE
    assert namespace == "user-projects-needing-attention"
    ...
    for src, label in (
        ...
        assert (
            "_USER_PROJECTS_NEEDING_ATTENTION_CACHE_NAMESPACE" in s
        ), (
            f"_USER_PROJECTS_NEEDING_ATTENTION_CACHE_NAMESPACE "
