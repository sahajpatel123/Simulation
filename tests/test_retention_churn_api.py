"""Route-level tests for the /simulations/{id}/retention-churn endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_retention_churn(
...
def test_completed_simulation_returns_retention_payload(
    ...
    assert 0.0 <= out.weighted_day30_survival <= 1.0
    assert 0.0 <= out.weighted_day90_survival <= 1.0
    assert out.highest_churn_stage in {
        "day1",
        "day7",
        "day30",
        "day90",
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("Weekly usage habit drives retention"),
        ],
    )
    ...
    assert any("retention" in a["text"] for a in run["assumptions"])
