"""Route-level tests for the /me/quick-stats cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_quick_stats(
        db=_FakeSession(),
        current_user=u,
    )


def test_quick_stats_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_quick_stats_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_quick_stats_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_quick_stats_namespace_constant_export():
...
def test_quick_stats_namespace_consistency_across_modules():
    ...
    namespace = users_mod._USER_QUICK_STATS_CACHE_NAMESPACE
    assert namespace == "user-quick-stats"
    ...
    for src, label in (
        ...
        assert (
            "_USER_QUICK_STATS_CACHE_NAMESPACE" in s
        ), (
            f"_USER_QUICK_STATS_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
