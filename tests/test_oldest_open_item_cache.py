"""Route-level tests for the /me/oldest-open-item cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_oldest_open_item(
        db=_FakeSession(),
        current_user=u,
    )


def test_oldest_open_item_caches_payload(monkeypatch):
    pytest.importorskip(
    ...
    out1 = _call_route()
    assert out1.oldest_age_days is None
...
def test_oldest_open_item_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_oldest_open_item_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_oldest_open_item_namespace_constant_export():
...
def test_oldest_open_item_namespace_consistency_across_modules():
    ...
    for src, label in (
        ...
        assert (
            "_USER_OLDEST_OPEN_ITEM_CACHE_NAMESPACE" in s
        ), (
            f"_USER_OLDEST_OPEN_ITEM_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
