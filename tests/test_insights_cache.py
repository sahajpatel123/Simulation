"""Route-level tests for the /me/insights cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_insights(
        db=_FakeSession(),
        current_user=u,
    )


def test_insights_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_insights_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_insights_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_insights_namespace_constant_export():
...
def test_insights_namespace_consistency_across_modules():
    ...
    for src, label in (
        ...
        assert (
            "_USER_INSIGHTS_CACHE_NAMESPACE" in s
        ), (
            f"_USER_INSIGHTS_CACHE_NAMESPACE not imported "
            f"in {label}"
        )
        assert (
