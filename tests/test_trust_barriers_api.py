"""Route-level tests for the /simulations/{id}/trust-barriers endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_trust_barriers(
...
def test_completed_simulation_returns_trust_barriers_payload(
    ...
    assert 0.0 <= out.trust_index <= 1.0
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("We are a known brand with many reviews"),
        ],
    )
    ...
    assert any("known brand" in a["text"] for a in run["assumptions"])
