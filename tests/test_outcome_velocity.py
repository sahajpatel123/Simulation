...
def test_public_allowlist_matches_callers():
    from app.simulation import outcome_velocity
    assert set(outcome_velocity.__all__) == {
...
def test_fast_verdict_when_under_24_hours():
    ...
    assert out["average_gap_hours"] == 9.0
...
def test_schema_default_shape():
    from app.schemas.user import OutcomeVelocityOut
    out = OutcomeVelocityOut()
    assert out.sample_count == 0
    assert out.average_gap_hours is None
    assert out.verdict == "INSUFFICIENT_DATA"
...
def test_schema_round_trip():
    ...
    out = OutcomeVelocityOut(**payload)
    assert out.sample_count == 1
    assert out.average_gap_hours == 12.0
    assert out.verdict == "FAST"
