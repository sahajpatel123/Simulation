"""Route-level tests for the /me/projects-summary cache.

Pins: cache hit short-circuit, per-user key isolation,
Redis-down no-op, and namespace-string consistency between
the read path (users.py) and the 4 invalidation sites
wired in this iteration (simulations.py, decisions.py,
outcomes.py, users.py clear-archive).
"""
...
def _call_route(current_user_id: int = 42):
    ...
    return users_mod.get_projects_summary(
        db=_FakeSession(),
        current_user=u,
    )


def test_projects_summary_caches_payload(monkeypatch) -> None:
    pytest.importorskip(
    ...
    out1 = _call_route()
    # Empty project list under the fake query mocks.
...
def test_projects_summary_cache_isolated_per_user(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_projects_summary_noop_when_redis_down(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_projects_summary_namespace_constant_export() -> None:
...
def test_projects_summary_namespace_consistency_across_modules() -> None:
    ...
    namespace = users_mod._USER_PROJECTS_SUMMARY_CACHE_NAMESPACE
    assert namespace == "user-projects-summary"
    ...
    for src, label in (
        ...
        assert (
            "_USER_PROJECTS_SUMMARY_CACHE_NAMESPACE" in s
        ), (
            f"_USER_PROJECTS_SUMMARY_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
