"""Route-level tests for the /me/sim-failure-rate cache.
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_sim_failure_rate(
        db=_FakeSession(),
        current_user=u,
    )


def test_sim_failure_rate_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_sim_failure_rate_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_sim_failure_rate_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_sim_failure_rate_namespace_constant_export():
...
def test_sim_failure_rate_namespace_consistency_across_modules():
    ...
    for src, label in (
        ...
        assert (
            "_USER_SIM_FAILURE_RATE_CACHE_NAMESPACE" in s
        ), (
            f"_USER_SIM_FAILURE_RATE_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
