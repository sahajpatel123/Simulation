...
def test_export_brief_completed_count_invalid_format_rejected_with_422() -> None:
    from app.api.v1 import projects as proj_mod
    from app.core.deps import get_current_user, get_db
    ...
    with TestClient(mini_app) as client:
        resp = client.get(
            "/projects/10/brief-completed-count/export",
            params={"format": "xlsx"},
        )
...
def test_export_landing_url_count_format_json_returns_payload() -> None:
    from app.api.v1 import projects as proj_mod
    ...
    body = _body(resp).decode("utf-8")
    assert '"project_id": 10' in body
    assert '"landing_url_count": 0' in body


def test_export_landing_url_count_missing_project_raises_404() -> None:
    from app.api.v1 import projects as proj_mod
    ...
    with pytest.raises(HTTPException) as exc:
        proj_mod.export_landing_url_count(
            project_id=10,
            format="csv",
...
def test_export_description_count_returns_csv_zero_when_missing() -> None:
    from app.api.v1 import projects as proj_mod

    db = _FakeSession(simulations=[])
    resp = proj_mod.export_description_count(
        project_id=10,
        format="csv",
        db=db,
        current_user=type("U", (), {"id": 42})(),
    ...
    assert resp.media_type == "text/csv; charset=utf-8"
    body = _body(resp).decode("utf-8")
    assert "project_id,description_count" in body
    assert "10,0" in body


def test_export_description_count_returns_csv_one_when_present() -> None:
    from app.api.v1 import projects as proj_mod

    class ProjectWithDescription(_Project):
        description = "A compelling project description"

    class SessionWithDescription(_FakeSession):
        def query(self, model, *args, **kwargs):
            name = getattr(model, "__name__", "")
            if name == "Project":
                return _FakeQuery([ProjectWithDescription()])
            return _FakeQuery(self.simulations)

    db = SessionWithDescription(simulations=[])
    resp = proj_mod.export_description_count(
        project_id=10,
        format="csv",
        db=db,
...
def test_export_description_count_format_json_returns_payload() -> None:
    from app.api.v1 import projects as proj_mod
    ...
    body = _body(resp).decode("utf-8")
    assert '"project_id": 10' in body
    assert '"description_count": 0' in body


def test_export_description_count_invalid_format_rejected_with_422() -> None:
    from app.api.v1 import projects as proj_mod
    from app.core.deps import get_current_user, get_db
    ...
    with TestClient(mini_app) as client:
        resp = client.get(
            "/projects/10/description-count/export",
            params={"format": "xlsx"},
        )
...
def test_export_description_count_missing_project_raises_404() -> None:
    from app.api.v1 import projects as proj_mod
    ...
    with pytest.raises(HTTPException) as exc:
        proj_mod.export_description_count(
            project_id=10,
            format="csv",
            db=NoProjectSession(simulations=[]),
            current_user=type("U", (), {"id": 42})(),
