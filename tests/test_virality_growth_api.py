"""Route-level tests for the /simulations/{id}/virality-growth endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_virality_growth(
...
def test_completed_simulation_returns_virality_growth_payload(
    ...
    assert 0.0 <= out.weighted_viral_coefficient <= 2.0
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("Referrals double when users can share output"),
        ],
    )
    ...
    assert any("share output" in a["text"] for a in run["assumptions"])
