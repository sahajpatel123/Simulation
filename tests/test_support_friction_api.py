"""Route-level tests for the /simulations/{id}/support-friction endpoint.
"""
...
def _call_route(
    *,
    ...
    return sim_mod.get_support_friction(
...
def test_completed_simulation_returns_support_friction_payload(
    ...
    assert 0.0 <= out.friction_index <= 1.0
    assert len(out.cluster_profiles) >= 50
    assert len(out.levers) == 6
    assert out.recommendations
    assert out.estimated_monthly_contacts_per_10k_users >= 0
...
def test_route_feeds_project_assumptions_into_conductor(
    ...
    session = _FakeSession(
        _FakeSimulation(),
        assumptions=[
            _FakeAssumption("We have a large knowledge base and docs"),
        ],
    )
    ...
    assert any("knowledge base" in a["text"] for a in run["assumptions"])
