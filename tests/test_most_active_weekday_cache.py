"""Route-level tests for the /me/most-active-weekday cache.
...
modules (simulations.py, decisions.py, outcomes.py,
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_most_active_weekday(
        db=_FakeSession(),
        current_user=u,
    )


def test_most_active_weekday_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_most_active_weekday_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_most_active_weekday_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_most_active_weekday_namespace_constant_export():
...
def test_most_active_weekday_namespace_consistency_across_modules():
    """Pin that the 3 invalidation modules (simulations.py,
    decisions.py, outcomes.py) import and use the constant
    ...
    namespace = users_mod._USER_MOST_ACTIVE_WEEKDAY_CACHE_NAMESPACE
    assert namespace == "user-most-active-weekday"
    ...
    for src, label in (
        ...
        assert (
            "_USER_MOST_ACTIVE_WEEKDAY_CACHE_NAMESPACE" in s
        ), (
            f"_USER_MOST_ACTIVE_WEEKDAY_CACHE_NAMESPACE "
