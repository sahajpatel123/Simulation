"""Route-level tests for the /me/outcome-velocity cache.
...
modules (simulations.py, outcomes.py).
"""
...
def _call_route(current_user_id=42):
    ...
    return users_mod.get_outcome_velocity(
        db=_FakeSession(),
        current_user=u,
    )


def test_outcome_velocity_caches_payload(monkeypatch):
    pytest.importorskip(
...
def test_outcome_velocity_cache_isolated_per_user(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_outcome_velocity_noop_when_redis_down(monkeypatch):
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_outcome_velocity_namespace_constant_export():
...
def test_outcome_velocity_namespace_consistency_across_modules():
    """Pin that both invalidation sites (simulations.py
    POST /simulations, outcomes.py POST /outcomes +
    DELETE /outcomes + POST outcome-feedback) import
    ...
    namespace = users_mod._USER_OUTCOME_VELOCITY_CACHE_NAMESPACE
    assert namespace == "user-outcome-velocity"
    ...
    for src, label in (
        ...
        assert (
            "_USER_OUTCOME_VELOCITY_CACHE_NAMESPACE" in s
        ), (
            f"_USER_OUTCOME_VELOCITY_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
