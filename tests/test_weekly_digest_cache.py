"""Route-level tests for the /me/weekly-digest cache.

Pins: cache hit short-circuit, per-user key isolation,
Redis-down no-op, and namespace-string consistency
between the read path and the 5 invalidation sites
wired in this iteration (POST /simulations,
POST /decisions, POST /outcome-feedback, POST /outcomes,
...
def _call_digest(current_user_id: int = 42):
    from app.api.v1 import users as users_mod

    u = _FakeUser()
    u.id = current_user_id

    return users_mod.get_weekly_digest(
        db=_FakeSession(),
        current_user=u,
    )


def test_weekly_digest_caches_payload(monkeypatch) -> None:
    pytest.importorskip(
...
def test_weekly_digest_cache_isolated_per_user(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
    ...
    _call_digest(current_user_id=42)
    _call_digest(current_user_id=99)
...
def test_weekly_digest_noop_when_redis_down(monkeypatch) -> None:
    pytest.importorskip(
        "scipy", reason="Route registration requires scipy",
    )
    import sys
...
def test_weekly_digest_namespace_constant_export() -> None:
...
def test_weekly_digest_namespace_consistency_across_modules() -> None:
    ...
    for src, label in (
        ...
        assert (
            "_USER_WEEKLY_DIGEST_CACHE_NAMESPACE" in s
        ), (
            f"_USER_WEEKLY_DIGEST_CACHE_NAMESPACE not "
            f"imported in {label}"
        )
        assert (
