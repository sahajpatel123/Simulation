"""Route-level tests for the /projects/{id}/mvp-features/export endpoint."""
...
class _Project:
    def __init__(self) -> None:
        self.id = 10
        self.mvp_feature_list = ["Auth", "Billing"]
...
def _call_route(
    *,
    ...
    return proj_mod.export_mvp_features(
...
def test_export_mvp_features_returns_csv() -> None:
    resp = _call_route()

    assert resp.media_type == "text/csv; charset=utf-8"
    assert 'filename="mvp-features-10.csv"' in resp.headers["Content-Disposition"]
    body = _body(resp).decode("utf-8")
    assert "index,feature" in body
    assert "1,Auth" in body
    assert "2,Billing" in body
    assert "user_id,42" in body


def test_export_mvp_features_format_json_returns_payload() -> None:
...
def test_export_mvp_features_empty_returns_header_only() -> None:
    session = _FakeSession(_EmptyProject())
    resp = _call_route(session=session)

    body = _body(resp).decode("utf-8")
    assert "index,feature" in body
    assert "1,Auth" not in body


def test_export_mvp_features_missing_project_raises_404() -> None:
