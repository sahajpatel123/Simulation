...
import { Plus, Zap, FolderOpen, X, ArrowRight, Loader2 } from 'lucide-react'
import Link from 'next/link'

const statusConfig: Record<SimulationStatus, { label: string; color: string; dot: string }> = {
  [SimulationStatus.DRAFT]: { label: 'Draft', color: 'text-slate-400 bg-slate-400/10 border-slate-400/20', dot: 'bg-slate-400' },
  [SimulationStatus.ASSUMPTIONS_EXTRACTED]: { label: 'Assumptions ready', color: 'text-amber-400 bg-amber-400/10 border-amber-400/20', dot: 'bg-amber-400' },
  [SimulationStatus.PROTOTYPE_GENERATED]: { label: 'Prototype ready', color: 'text-blue-400 bg-blue-400/10 border-blue-400/20', dot: 'bg-blue-400' },
  [SimulationStatus.ENVIRONMENT_SET]: { label: 'Environment set', color: 'text-purple-400 bg-purple-400/10 border-purple-400/20', dot: 'bg-purple-400' },
  [SimulationStatus.RUNNING]: { label: 'Running', color: 'text-indigo-400 bg-indigo-400/10 border-indigo-400/20', dot: 'bg-indigo-400 animate-pulse' },
  [SimulationStatus.COMPLETED]: { label: 'Completed', color: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20', dot: 'bg-emerald-400' },
  [SimulationStatus.FAILED]: { label: 'Failed', color: 'text-red-400 bg-red-400/10 border-red-400/20', dot: 'bg-red-400' },
}
    ...
    <div className="p-8">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{projects.length} simulation{projects.length !== 1 ? 's' : ''} in progress</p>
        </div>
      ...
      {projects.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          <p className="text-slate-500 text-sm mb-6 max-w-xs">Describe your idea and run it through 10,000 synthetic consumers before committing.</p>
          <button onClick={() => setShowModal(true)}
              ...
              <motion.div key={project.id}
                ...
                <Link href={`/project/${project.id}`}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-slate-600 text-xs">{new Date(project.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
              ...
              <div className="glass rounded-2xl p-8 border border-white/10">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-500 text-xs mt-0.5">TheCee will extract hidden assumptions automatically</p>
                  </div>
                ...
                <textarea
                  ...
                  placeholder="Describe your product idea, business model, or decision in plain language. The more detail you give, the more accurate the simulation..."
                  rows={5}
                ...
                <div className="flex items-center gap-3 justify-end">
                  ...
                  <button onClick={handleCreate} disabled={!idea.trim() || creating}
                    ...
                    {creating ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Extracting...</> : <>Run simulation <ArrowRight className="w-3.5 h-3.5" /></>}
                  </button>
                </div>
              </div>
            </motion.div>
